...
function Calendar(obj) {
	var rval = obj.value;
	if (rval.length == 8){
		rval = rval.substring(0,4) + "-" + rval.substring(4,6) + "-" + rval.substring(6,8);
	}													// jucke
	var now = rval.split("-");
	var x, y;
...
function doClick() {															// 날자를 선택하였을 경우
	...
	if (cal_Day.length > 7) {                                                // 날자 값이있으면
		var arval = cal_Day.split("-");
		var rval = arval[0] + '' + arval[1] + '' + arval[2];
		target.value = rval;													// 값 설정
	}
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML = "<TABLE WIDTH=120 class=search-box BORDER=0 CELLPADDING=0 CELLSPACING=0 ONMOUSEOVER=doOver(); ONMOUSEOUT=doOut(); STYLE='font-size:8pt;font-family:Tahoma;'>"
			...
			+ "<TR ALIGN=CENTER BGCOLOR=ThreedFace STYLE='color:Black;font-weight:bold;' ><TD class=admlist>일</TD><TD class=admlist>월</TD><TD class=admlist>화</TD><TD class=admlist>수</TD><TD class=admlist>목</TD><TD class=admlist>금</TD><TD class=admlist>토</TD></TR>";
